...
import {
  Share2, Copy, Eye, Trash2, Gift, WifiOff,
  CheckCheck, QrCode, Download, X, Loader2,
} from "lucide-react";
...
function WishQRPanel({ wish, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/qr?url=${encodeURIComponent(wishUrl)}`)
      .then((r) => r.json())
      .then((d) => { setQrSrc(d.dataUrl); setLoading(false) })
  ...
  const shareQR = async () => {
    ...
    try {
      ...
      const file = new File([blob], `wish-${wish.recipient}-qr.png`, { type: "image/png" })
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({ files: [file], title: `QR wish for ${wish.recipient}` })
      } else {
        downloadQR()
      }
    } catch {
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="relative w-36 h-36">
            <div className="w-full h-full rounded-xl overflow-hidden border border-stone-100 shadow-md">
              {qrSrc && <img src={qrSrc} alt="QR" className="w-full h-full object-cover" />}
            </div>
            {[
              ...
              <div key={i} className={`absolute w-4 h-4 border-[#1D9E75] ${cls}`} />
            ))}
          </div>
          <p className="text-[10px] text-stone-400">Scan with any camera to reveal</p>
      ...
      <div className="max-w-[430px] mx-auto px-4 pb-36">
        ...
        {!loaded && (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-28 bg-stone-200/50 rounded-2xl animate-pulse" />
            ))}
          </div>
        )}
        ...
        {loaded && wishes.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center px-4">
            ...
            <h2 className="text-[17px] font-bold text-stone-700 mb-2">{t("myWishes.empty")}</h2>
            <p className="text-[13px] text-stone-400 mb-7 leading-relaxed max-w-[260px]">{t("myWishes.emptyDesc")}</p>
            <Link href={`/${locale}`}>
              <Button className="rounded-xl bg-[#1D9E75] hover:bg-[#178060] text-[13px] px-6 h-11 shadow-md shadow-[#1D9E75]/20">
        ...
        {loaded && wishes.length > 0 && (
          <div className="space-y-3">
            ...
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-2xl px-4 py-3">
              ...
              <p className="text-[11px] text-amber-700 leading-relaxed">{t("myWishes.deviceWarning")}</p>
            </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    transform: isDeleting ? "scale(0.97) translateY(-4px)" : "scale(1)",
                  }}
                >
                  <div
                  ...
                  >
                    <div className="p-4">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-0.5">
                            <p className="text-[14px] font-bold text-stone-800 truncate">{wish.recipient}</p>
                            <span
                          ...
                          <p className="text-[12px] text-stone-400 truncate leading-snug">{wish.message}</p>
                          <p className="text-[10px] text-stone-300 mt-1">{formatDate(wish.created_at)}</p>
                        </div>
                        <button
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          {copied === wish.id ? <><CheckCheck size={12} />{t("common.copied")}</> : <><Copy size={12} />{t("common.copy")}</>}
                        </button>
